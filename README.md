...
A single natural-language box turns any goal into an ordered, **multi-app** action plan that runs step-by-step with a live execution trace. Each step shows the app, the action, its parameters, the result, and whether it ran **live** or **simulated**. Includes example prompts and a `⌘⏎` shortcut to run.
...
2. **Planning.** The agent sends the goal + the catalog of *connected* apps to Claude, which returns an ordered, cross-app plan as strict JSON.
...
| **Integrations** | Slack Bolt (live) + a 23-app connector registry |
...
```
AgentOS/
└── chronicle-ai-main/                 # the AgentOS application
    ...
    └── src/index.css                  # Claude/Anthropic design system + animations
```
...
| `GET /api/integrations` | List all 23 connectors + connection status |
...
- Scheduled & event-triggered agent runs
